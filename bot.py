...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('axytehost_bot.log'),
...
WELCOME_MESSAGE = "Welcome To AxyteHost! Get Started With Us!"
MAX_VPS_PER_USER = int(os.getenv('MAX_VPS_PER_USER', '3'))
DEFAULT_OS_IMAGE = os.getenv('DEFAULT_OS_IMAGE', 'ubuntu:22.04')
DOCKER_NETWORK = os.getenv('DOCKER_NETWORK', 'bridge')
MAX_CONTAINERS = int(os.getenv('MAX_CONTAINERS', '100'))
DB_FILE = 'axytehost.db'
BACKUP_FILE = 'axytehost_backup.pkl'
...
RUN echo '{welcome_message}' > /etc/motd && \\
    ...
    echo 'axytehost-{vps_id}' > /etc/hostname
...
async def build_custom_image(vps_id, username, root_password, user_password, base_image=DEFAULT_OS_IMAGE):
    ...
    try:
        ...
        # Build the image
        image_tag = f"axytehost/{vps_id.lower()}:latest"
...
async def setup_container(container_id, status_msg, memory, username, vps_id=None, use_custom_image=False):
    ...
    try:
        ...
        hostname_cmd = f"echo 'axytehost-{vps_id}' > /etc/hostname && hostname axytehost-{vps_id}"
...
async def create_vps_command(ctx, memory: int, cpu: int, disk: int, owner: discord.Member, 
    ...
    try:
        ...
        if use_custom_image:
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    volumes={
                        f'axytehost-{vps_id}': {'bind': '/data', 'mode': 'rw'}
                    },
        ...
        try:
            embed = discord.Embed(title="🎉 Altarehost VPS Creation Successful", color=discord.Color.green())
...
"""
        ...
        process = await asyncio.create_subprocess_exec(
            "docker", "build", "-t", "axytehost-bot", "-f", "Dockerfile.bot", ".",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
        ...
        try:
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    detach=True,
                    privileged=True,
                    hostname=f"axytehost-{self.vps_id}",
                    mem_limit=memory_bytes,
                    cpu_period=100000,
                    ...
                    network=DOCKER_NETWORK,
                    volumes={
                        f'axytehost-{self.vps_id}': {'bind': '/data', 'mode': 'rw'}
                    }
                )
            except docker.errors.ImageNotFound:
